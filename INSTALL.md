...
6. **Диагностика** → нажмите «Проверить» — все пакеты должны быть зелёными (включая `pywinrm`/`impacket` для «Управления ПК аптек»)
7. Для маркировки: **Настройки → Учётные записи → УЗ Regime** (Basic Auth к ЛМ ЧЗ) и **Компании** → токен ЧЗ у нужной компании
