...
- Категории и статьи отдаются **без пагинации** — показываются все, сколько бы их ни было
- Список статей содержит только превью (`excerpt`); полный текст догружается при открытии статьи
  (иначе импортированные из `.docx` статьи с картинками в base64 весили бы сотни КБ на каждый заход в раздел)
...
| Метод | URL | Описание |
|-------|-----|----------|
| GET/POST | `/api/clients/faq-categories/` | Список / создание категорий (без пагинации) |
| GET/POST | `/api/clients/faq-articles/?category=ID&search=Q` | Список / создание статей. Список **без пагинации** и **без поля `content`** — вместо него `excerpt` (текстовое превью, 200 символов) |
| PUT/DELETE | `/api/clients/faq-articles/{id}/` | Обновление / удаление статьи |
| GET | `/api/clients/faq-articles/{id}/` | Статья целиком (единственный источник полного `content`) |
