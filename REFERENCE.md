...
### 🖥️ Управление ПК аптек (только Администратор)
Раздел для удалённого выполнения PowerShell/cmd-скриптов и копирования файлов на серверы и кассы (Windows-ПК) аптек. Приложение `apps/pcmgmt`, страница `PcManagementPage.jsx`, эндпоинты `/api/pcmgmt/*`.

**Две вкладки:**

| Вкладка | Содержимое |
|---------|------------|
| Клиенты | Список аптек с чекбоксами (выбор запоминается), фильтр по компании, поиск по адресу, переключатель режима: **Сервер / Кассы / Сервер + кассы** |
| Задания | Кнопка «Добавить задание», история запусков с онлайн-прогрессом, кнопки «Подробно» / «Отменить» / «Повторить» |
...
| Вкладка | Содержимое |
|---------|------------|
| Учётные записи | SSH (Микротик), SMTP (Email), УЗ для подключения к ПК аптек (WinRM/SMB/WMI) |
...
### Endpoints API Управление ПК аптек (только Администратор)
...
| POST | `/api/pcmgmt/jobs/` | Создать и запустить задание (multipart при копировании файла) |
...
│   │       ├── settings_views.py # Настройки SSH, SMTP и УЗ Windows (WinRM)
