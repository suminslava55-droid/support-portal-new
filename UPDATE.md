...
### Август 2026 — База знаний: показываются все статьи

- **Снят лимит в 20 статей.** У `FaqArticleViewSet` и `FaqCategoryViewSet` отключена пагинация DRF
  (`pagination_class = None`), которая по умолчанию отдавала только первую страницу из 20 записей.
  Кнопки «следующая страница» в разделе нет, поэтому всё сверх 20 было просто недоступно.
- **Список статей стал легче в ~120 раз** (780 КБ → 6.5 КБ). Раньше список отдавал полный HTML каждой статьи;
  у импортированных из `.docx` статей это сотни килобайт из-за картинок в base64 (одна статья — 685 КБ).
  Теперь в списке вместо `content` приходит `excerpt` — текстовое превью на 200 символов
  (`FaqArticleListSerializer`), а полный текст фронтенд запрашивает при открытии статьи.
- **Фронтенд** — клик по статье догружает её через `GET /api/clients/faq-articles/{id}/` со спиннером;
  превью в списке берётся из `excerpt`. Заодно исправлено: после редактирования открытая статья
  перечитывается, а не показывает старый текст.
...
### Август 2026 — ЧЗ (Честный знак) + улучшения «Управления ПК аптек»

#### Вкладка ЧЗ в карточке клиента
- **Живой статус ЛМ ЧЗ** (локальный модуль «Честный знак») на сервере аптеки (`server_ip:5995`, `GET /api/v2/status`): статус, режим, версия, serviceUrl, время обновления БД и синхронизации. В БД не пишется.
- **Баннер здоровья офлайн-проверок** — отсчёт до дедлайна **72ч** от `lastSync` (по истечении ЛМ уходит в `sync_error` и касса не может проверять маркировку офлайн).
- **Инициализация** ЛМ — `POST /api/v2/init` с Basic Auth (**УЗ Regime**) и токеном ЧЗ компании.
- **Перезагрузка службы ЧЗ** — кнопка (только при проблеме, роли admin/sysadmin) создаёт задание в «Управление ПК аптек»: PowerShell на сервере останавливает `regime`→`yenisei` (принудительно при зависании), ждёт 30с, запускает `regime`.
- **Токен ЧЗ** у компании (`OfdCompany.chz_token_encrypted`) и **УЗ Regime** в Настройки → Учётные записи (шифрование Fernet).

Новые файлы: `frontend/src/pages/client-detail/ClientDetailChz.jsx`, `frontend/src/components/ChzIcon.jsx`; в `backend/apps/clients/views/kkt_views.py` — `ClientChzView`, `ClientChzRestartView`.
Миграции: `clients/0039` (chz_token, regime_user/password), `pcmgmt/0005` (PcJob.name).

#### Управление ПК аптек
- **Название задания** — необязательное поле (колонка в списке, в заголовке «Подробно»).
- **Редактор скрипта** — подсветка синтаксиса PowerShell/cmd, без автопереноса длинных строк (горизонтальная прокрутка).
- Кнопка **«Новое задание»** на вкладке «Клиенты» — сразу открывает форму создания с выбранными аптеками.
- **Доступ расширен** на роль Системный администратор (раздел, эндпоинты `/api/pcmgmt/*` и перезагрузка ЧЗ переведены на `IsAdminOrSysadmin`).

**Деплой этого обновления:**
```bash
cd /opt/support-portal && git pull
docker compose exec backend python manage.py migrate   # clients/0039, pcmgmt/0005
docker compose restart backend
./deploy-frontend.sh
...
- **Умный повтор** — «Повторить» не запускается на ПК, где уже успешно выполнено (по всей цепочке повторов, поле `PcJob.repeat_of`), а кассы опрашивает заново: недоступная ранее касса с новым IP тоже получит задание. Повтор отталкивается от последнего прогона; в «Подробно» все прогоны цепочки показываются вкладками (результаты каждого сохраняются).
